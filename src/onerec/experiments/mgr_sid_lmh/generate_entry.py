#!/usr/bin/env python
"""Generate SID indices from a current LMH tokenizer checkpoint."""
...
def main() -> None:
    ...
    output = {str(i): all_indices_np[i].tolist() for i in range(len(all_indices_np))}
    ...
    summary = {
        ...
        "num_items": len(all_indices_np),
        "unique_sid": len(counts),
        "collision_count": len(all_indices_np) - len(counts),
        "collision_rounds": collision_round,
