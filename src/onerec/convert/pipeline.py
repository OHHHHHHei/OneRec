#!/usr/bin/env python3
"""
Convert dataset (Office/Industrial_and_Scientific) to OneRec format with semantic IDs
"""
...
def convert_interactions_to_csv(splits: Dict[str, List], items: Dict[str, Dict], 
    ...
    """Convert interaction data to OneRec CSV format using semantic IDs"""
...
def _resolve_info_path(config, output_dir: str, category: str) -> str:
    info_path = str(config.extras.get("info_path", "")).strip()
    if info_path:
        return info_path
    return str(Path(output_dir) / "info" / f"{category}_5_2016-10-2018-11.txt")


def run_convert(config) -> str:
    data_dir = config.data.data_dir
    dataset_name = config.data.dataset_name
    output_dir = config.data.output_dir or config.output.output_dir
    category = config.data.category or dataset_name
    if not data_dir:
        raise ValueError("convert requires data.data_dir")
    if not dataset_name:
        raise ValueError("convert requires data.dataset_name")
    if not output_dir:
        raise ValueError("convert requires data.output_dir or output.output_dir")

    print(f"Loading {dataset_name} data from {data_dir}")
    dataset_data = load_dataset(data_dir, dataset_name)
    ...
    info_file = _resolve_info_path(config, output_dir, category)
    ...
    for split_name in ["train", "valid", "test"]:
        if split_name not in dataset_data["splits"]:
            continue
        ...
        convert_interactions_to_csv(
            ...
            split_output_dir,
            category,
            max_valid_samples=config.extras.get("max_valid_samples"),
            max_test_samples=config.extras.get("max_test_samples"),
            seed=int(config.training.seed),
            keep_longest_only=bool(config.extras.get("keep_longest_only", False)),
        )

    print(f"\nConversion completed! Data saved to {output_dir}")
    return output_dir

def main():
    parser = argparse.ArgumentParser(description='Convert dataset (Office_Products/Industrial_and_Scientific) to OneRec format with semantic IDs')
    ...
    parser.add_argument('--output_dir', type=str,
                       help='Output directory for OneRec format data')
    ...
    print(f"You can now use these files with OneRec training scripts by setting category='{args.category}'")
