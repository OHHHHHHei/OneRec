...
# Dataset name mapping (Optional for 14, kept for compatibility if needed)
...
def load_metadata_json2csv_style(category, metadata_file=None):
    """Load metadata using json2csv style processing, supporting Python eval format"""
    if metadata_file is None:
        print(f"Error: metadata_file must be provided for Amazon 14 processing")
        return [], {}, set()
        ...
        with open(metadata_file, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                try:
                    ...
                    try:
                        # Fallback to python eval (for Amazon 14 meta files)
                        # reckless but necessary for this dataset format
                        meta = eval(line)
...
def load_reviews_json2csv_style(category, reviews_file=None, start_timestamp=None, end_timestamp=None):
    ...
    if reviews_file is None:
        print(f"Error: reviews_file must be provided for Amazon 14 processing")
        return []

    try:
        with open(reviews_file, 'r', encoding='utf-8', errors='ignore') as f:
             # Amazon 14 reviews are typically valid JSON lines
...
def k_core_filtering_json2csv_style(reviews, id_title, K=5, start_timestamp=None, end_timestamp=None):
    ...
    while True:
        ...
        for review in tqdm(reviews, desc=f"K-core filtering (Iter {loop_count})"):
        ...
        print(f"Iter {loop_count}: Users={len(user_counts)}, Items={len(item_counts)}, Reviews={total}")
        if flag:
            print(f"  Removing {current_remove_users} users and {current_remove_items} items below K={K}")
...
def generate_interaction_list_json2csv_style(reviews, user2index, item2index, id_title):
    ...
    for review in tqdm(reviews, desc="Building interaction list"):
        ...
        # Only use title if present, otherwise empty string (though filtering should prevent this)
        interact[user]['titles'].append(id_title.get(item, ""))
    
    # Sort by timestamp for each user
    interaction_list = []
    
    # Pre-calculate to avoid re-allocating
    valid_users = [user for user in interact.keys() if len(interact[user]['items']) > 1]
    
    for user in tqdm(valid_users, desc="Creating interaction sequences"):
...
def convert_to_atomic_files_json2csv_style(args, interaction_list, user2index):
    ...
    # Write train file
    with open(os.path.join(output_dir, f'{args.dataset}.train.inter'), 'w', encoding='utf-8') as file:
    ...
    # Write valid file  
    with open(os.path.join(output_dir, f'{args.dataset}.valid.inter'), 'w', encoding='utf-8') as file:
        file.write('user_id:token\titem_id_list:token_seq\titem_id:token\n')
    ...
    # Write test file
    with open(os.path.join(output_dir, f'{args.dataset}.test.inter'), 'w', encoding='utf-8') as file:
        file.write('user_id:token\titem_id_list:token_seq\titem_id:token\n')
...
def create_item_features_amazon18_style(metadata, item2index, id_title):
    ...
    for item_asin, item_id in item2index.items():
        if item_asin in asin_to_meta:
            ...
            if descriptions:
                # meta['description'] in Amazon 14 might be a string
                descriptions = clean_text(str(descriptions))
            else:
                descriptions = ""
            ...
            if categories and len(categories) > 0:
                ...
                # Amazon 14 categories are typically list of lists: [['Electronics', 'Computers']]
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='Arts', help='Dataset name (e.g., Beauty)')
    ...
    parser.add_argument('--metadata_file', type=str, required=True, help='metadata file path')
    parser.add_argument('--reviews_file', type=str, required=True, help='reviews file path')
...
if __name__ == '__main__':
    ...
    output_dataset_dir = os.path.join(args.output_path, args.dataset)
    write_json_file(user2items_final, os.path.join(output_dataset_dir, f'{args.dataset}.inter.json'))
    ...
    write_json_file(item2feature, os.path.join(output_dataset_dir, f'{args.dataset}.item.json'))
    write_json_file(review_data, os.path.join(output_dataset_dir, f'{args.dataset}.review.json'))
    
    write_remap_index(user2index, os.path.join(output_dataset_dir, f'{args.dataset}.user2id'))
    write_remap_index(item2index, os.path.join(output_dataset_dir, f'{args.dataset}.item2id'))
