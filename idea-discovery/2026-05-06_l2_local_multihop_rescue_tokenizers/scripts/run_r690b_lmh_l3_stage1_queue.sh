...
if [[ "$#" -lt 3 ]]; then
  echo "Usage: $0 <gpu_id> <queue_name> <l3_weight> [<l3_weight> ...]" >&2
  exit 2
fi
...
CONFIG_DIR="$BRANCH_DIR/configs/stage1_l3_sweep"
CKPT_BASE="/data/leejt/OneRec/output_weights/experiments/mgr_sid_l3_lmh_sweep_20260508"
INDEX_DIR="$CKPT_BASE/generated_indices"
LOG_DIR="$ROOT_DIR/logs/l2_lmh_l3_stage1"
...
write_config() {
  ...
  python - "$BASE_CONFIG" "$config_path" "$l3_weight" "$ckpt_dir" <<'PY'
import sys
import yaml

base_config, out_path, l3_weight, ckpt_dir = sys.argv[1:5]
with open(base_config) as f:
    cfg = yaml.safe_load(f)

cfg["ckpt_dir"] = ckpt_dir

# Keep this stage focused on the current R690b LMH anchor:
# L1 semantic routing stabilizer + L2 local_multihop InfoNCE at 0.010.
...
cfg["l3_contrastive_pull_weight"] = float(l3_weight)
...
echo "[L3-QUEUE:$QUEUE_NAME] GPU=$GPU_ID l3_weights=$*"

for l3_weight in "$@"; do
  suffix="$(weight_suffix_milli "$l3_weight")"
  tag="r690b_lmh_l2_weight001_l3_weight${suffix}"
  ...
  echo "============================================"
  echo "[L3-QUEUE:$QUEUE_NAME] l3_weight=$l3_weight tag=$tag"
  echo "[L3-QUEUE:$QUEUE_NAME] config=$config_path"
  echo "[L3-QUEUE:$QUEUE_NAME] ckpt=$ckpt_dir"
  echo "============================================"

  if [[ -s "$generate_summary" && -s "$index_file" ]]; then
    echo "[L3-QUEUE:$QUEUE_NAME] existing generated artifacts found, skip training."
  else
    write_config "$l3_weight" "$ckpt_dir" "$config_path"
    mkdir -p "$ckpt_dir"
  ...
  echo "[L3-QUEUE:$QUEUE_NAME] structure diagnostics:"
  diagnose_index "$index_file" "$structure_summary"

  if ! passes_gate "$structure_summary"; then
    echo "[L3-QUEUE:$QUEUE_NAME] gate failed after $tag; stop this queue to avoid wasting GPU."
    exit 0
  fi
done

echo "[L3-QUEUE:$QUEUE_NAME] completed."
