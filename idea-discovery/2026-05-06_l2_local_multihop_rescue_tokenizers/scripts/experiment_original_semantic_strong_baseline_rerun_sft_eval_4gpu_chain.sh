...
if [[ "$NPROC" -ne 4 ]]; then
  echo "ERROR: this strong baseline rerun chain is calibrated for 4 GPUs; got GPU_LIST=$GPU_LIST (nproc=$NPROC)" >&2
  exit 1
fi
...
VARIANT="original_semantic_title_off_desc_p05_rerun_4gpu"
...
echo "[STRONG-BASELINE-RERUN] recipe=title_history2sid_off + desc_align_p05"
echo "[STRONG-BASELINE-RERUN] original SID=data/Amazon/index/Industrial_and_Scientific.index.json"
echo "[STRONG-BASELINE-RERUN] effective_batch=1024 micro_batch=2 world_size=$NPROC grad_accum=$((1024 / (2 * NPROC))) seed=42"
echo "[STRONG-BASELINE-RERUN] start SFT on GPUs $GPU_LIST"
...
echo "[STRONG-BASELINE-RERUN] start evaluate on GPUs $GPU_LIST"
...
echo "[STRONG-BASELINE-RERUN] completed: $RESULT_PATH"
