...
if [[ "$#" -lt 3 ]]; then
  echo "Usage: $0 <gpu_id> <queue_name> <l2_weight> [<l2_weight> ...]" >&2
  exit 2
fi
...
CONFIG_DIR="$BRANCH_DIR/configs/stage1_l2_sweep"
...
write_config() {
  ...
  python - "$BASE_CONFIG" "$config_path" "$weight" "$ckpt_dir" <<'PY'
import sys
import yaml

base_config, out_path, weight, ckpt_dir = sys.argv[1:5]
with open(base_config) as f:
    cfg = yaml.safe_load(f)

cfg["l2_contrastive_pull_weight"] = float(weight)
cfg["ckpt_dir"] = ckpt_dir

# Keep this stage focused on the successful R690b LMH setup.
...
echo "[QUEUE:$QUEUE_NAME] GPU=$GPU_ID weights=$*"

for weight in "$@"; do
  suffix="$(weight_suffix "$weight")"
  tag="r690b_lmh_l2_contrastive_pull_weight${suffix}"
  ...
  echo "============================================"
  echo "[QUEUE:$QUEUE_NAME] weight=$weight tag=$tag"
  ...
  if [[ -s "$generate_summary" && -s "$index_file" ]]; then
    ...
    write_config "$weight" "$tag" "$ckpt_dir" "$config_path"
    mkdir -p "$ckpt_dir"
