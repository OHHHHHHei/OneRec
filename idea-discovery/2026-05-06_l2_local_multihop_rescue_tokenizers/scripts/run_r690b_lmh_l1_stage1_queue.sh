...
if [[ "$#" -lt 3 ]]; then
  echo "Usage: $0 <gpu_id> <queue_name> <l1_weight> [<l1_weight> ...]" >&2
  exit 2
fi
...
CONFIG_DIR="$BRANCH_DIR/configs/stage1_l1_sweep"
CKPT_BASE="/data/leejt/OneRec/output_weights/experiments/mgr_sid_l1_lmh_sweep_20260508"
INDEX_DIR="$CKPT_BASE/generated_indices"
LOG_DIR="$ROOT_DIR/logs/l2_lmh_l1_stage1"
...
write_config() {
  ...
  python - "$BASE_CONFIG" "$config_path" "$l1_weight" "$ckpt_dir" <<'PY'
import sys
import yaml

base_config, out_path, l1_weight, ckpt_dir = sys.argv[1:5]
with open(base_config) as f:
    cfg = yaml.safe_load(f)

cfg["ckpt_dir"] = ckpt_dir

# L1 sweep: keep the validated R690b LMH mainline fixed except for
# the semantic L1 routing stabilizer.
cfg["coarse_weight"] = 0.0
cfg["mid_weight"] = 0.0
cfg["local_weight"] = 0.0
cfg["l1_contrastive_pull_weight"] = float(l1_weight)
...
echo "[L1-QUEUE:$QUEUE_NAME] GPU=$GPU_ID l1_weights=$*"

for l1_weight in "$@"; do
  suffix="$(weight_suffix_milli "$l1_weight")"
  tag="r690b_lmh_l1_weight${suffix}_l2_weight001_l3_weight002"
  ...
  echo "============================================"
  echo "[L1-QUEUE:$QUEUE_NAME] l1_weight=$l1_weight tag=$tag"
  echo "[L1-QUEUE:$QUEUE_NAME] config=$config_path"
  echo "[L1-QUEUE:$QUEUE_NAME] ckpt=$ckpt_dir"
  echo "============================================"

  if [[ -s "$generate_summary" && -s "$index_file" ]]; then
    echo "[L1-QUEUE:$QUEUE_NAME] existing generated artifacts found, skip training."
  else
    write_config "$l1_weight" "$ckpt_dir" "$config_path"
    mkdir -p "$ckpt_dir"
  ...
  echo "[L1-QUEUE:$QUEUE_NAME] structure diagnostics:"
  diagnose_index "$index_file" "$structure_summary"

  if ! passes_gate "$structure_summary"; then
    echo "[L1-QUEUE:$QUEUE_NAME] gate failed after $tag; stop this queue to avoid wasting GPU."
    exit 0
  fi
done

echo "[L1-QUEUE:$QUEUE_NAME] completed."
