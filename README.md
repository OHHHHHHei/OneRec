...
The repository now includes a refactored mainline implementation under `src/minionerec` with a unified CLI:
...
Compatibility wrappers are still available through the original root entrypoints such as `sft.py`, `rl.py`, `evaluate.py`, and `convert_dataset.py`.
