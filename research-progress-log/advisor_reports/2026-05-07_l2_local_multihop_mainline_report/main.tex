...
\subsection{SFT 多次重复运行稳定性分析}

为了判断当前提升是否只是单次 SFT（监督微调）训练的随机波动，我们进一步对原版 Baseline 和当前主线 R690b-LMH 做了三次同配置 rerun（重复运行）对比。这里的 rerun 使用相同 seed（随机种子）和相同代码/配置；但由于多卡训练、DeepSpeed（分布式训练框架）和底层算子的非完全确定性，同 seed 不等于完全 deterministic（确定性）。

实验对象如下：Baseline 使用原版 semantic SID（语义 SID）和原版较强 SFT 配方 \texttt{title\_history2sid\_off + desc\_align\_p05}；当前主线使用 R690b-LMH tokenizer（分词器）和它自身最优 SFT 配方 \texttt{title\_history2sid\_on + desc\_align\_p05}。
...
Baseline 均值$\pm$标准差 & 0.06574$\pm$0.00117 & 0.08337$\pm$0.00169 & 0.09089$\pm$0.00213 & 0.10201$\pm$0.00153 \\
\midrule
R690b-LMH run 1 & 0.07059 & 0.08813 & 0.09489 & 0.10438 \\
R690b-LMH run 2 & 0.06861 & 0.08495 & 0.09282 & 0.10272 \\
R690b-LMH run 3 & 0.07126 & 0.08463 & 0.09180 & 0.10163 \\
R690b-LMH 均值$\pm$标准差 & 0.07015$\pm$0.00138 & 0.08590$\pm$0.00194 & 0.09317$\pm$0.00158 & 0.10291$\pm$0.00139 \\
\midrule
平均相对提升 & +6.71\% & +3.04\% & +2.51\% & +0.89\% \\
\bottomrule
\end{tabular}
\end{center}
...
Baseline 均值$\pm$标准差 & 0.06574$\pm$0.00117 & 0.09655$\pm$0.00230 & 0.11486$\pm$0.00331 & 0.14935$\pm$0.00145 \\
\midrule
R690b-LMH run 1 & 0.07059 & 0.10082 & 0.11736 & 0.14692 \\
R690b-LMH run 2 & 0.06861 & 0.09685 & 0.11604 & 0.14670 \\
R690b-LMH run 3 & 0.07126 & 0.09442 & 0.11185 & 0.14251 \\
R690b-LMH 均值$\pm$标准差 & 0.07015$\pm$0.00138 & 0.09736$\pm$0.00323 & 0.11508$\pm$0.00288 & 0.14538$\pm$0.00249 \\
\midrule
平均相对提升 & +6.71\% & +0.84\% & +0.19\% & -2.66\% \\
\bottomrule
\end{tabular}
\end{center}

重复运行结果说明：当前主线不是单次抽样偶然。三次平均后，R690b-LMH 在 NDCG@1/3/5/10 上均超过 Baseline，尤其 NDCG@1 提升 6.71\%，说明它主要改善 top-rank ranking quality（前排排序质量）。但 HR@10 低于 Baseline 2.66\%，说明它并没有同步提升 recall coverage（召回覆盖），甚至可能把概率质量更集中地压到前排，牺牲了一部分长尾命中。因此，当前结果应保守表述为：主线提高了 SFT 阶段的前排排序质量，而不是全面超过原版 MiniOneRec。
